...
class MediaService:
    ...
    def refresh_availability(self, raid_id: UUID | str) -> RaidMediaIndex:
        raid = self._require_raid(raid_id)
        index = self._load_index(raid)
        index.recordings = [
            item.model_copy(update={"available": item.canonical_path.is_file()})
    ...
    def _wait_for_stability(self, path: Path) -> None:
        deadline = time.monotonic() + self._settings.file_stability_timeout_seconds
        ...
        while True:
            try:
                ...
                raise MediaPathError(f"Unable to inspect media file: {path}") from exc
            ...
            if time.monotonic() >= deadline:
                raise MediaFinalizationError(
                    "Recording did not become stable before the configured timeout"
                )
    ...
    def _validate_source_path(self, path: Path) -> None:
        ...
        raise MediaPathError(
            "Media path is outside data_root and api.allowed_evidence_roots"
        )
        ...
        for recording in reversed(index.recordings):
            ...
            if same_path and recording.size_bytes == size:
                return recording.model_copy(
                    update={"available": recording.canonical_path.is_file()}
                )
        return None
    ...
    def _probe_payload(payload: object) -> dict[str, Any]:
        ...
        if not isinstance(streams_data, list):
            streams_data = []
        streams = [item for item in streams_data if isinstance(item, dict)]
        video = next(
        ...
        duration = MediaService._optional_float(format_data.get("duration"))
        fps = None
    ...
    def _run_ffmpeg(
        ...
        if result.returncode != 0 or not destination.is_file():
            ...
            detail = result.stderr.strip() or "ffmpeg did not create a clip"
    ...
    def _load_index(self, raid: RaidRecord) -> RaidMediaIndex:
        ...
        return RaidMediaIndex.model_validate_json(path.read_text(encoding="utf-8"))
    ...
    def _require_enabled(self) -> None:
        if not self.enabled:
            raise MediaDisabledError(
                "The Media Assistance subsystem is disabled in configuration"
            )
