...
class ObsRecordingController:
    ...
    def _get_client(self) -> object:
        ...
        except Exception as exc:  # SDK exposes several transport-specific exception types.
            raise ObsIntegrationError(f"Unable to connect to OBS WebSocket: {exc}") from exc
        return self._client
    ...
    def status(self) -> RecordingStatus:
        try:
            ...
            raise ObsIntegrationError(f"Unable to read OBS recording status: {exc}") from exc
