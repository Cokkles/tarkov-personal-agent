...
class DesktopWindow(
    ...
    def _build_window(self) -> None:
        ...
        self._add_page("ppe", self._feature_page("PERSONAL PLAYSTYLE ENGINE", "Profile dimensions, evidence, confidence, and long-term trends.", "/ppe"))
        self._add_page("media", self._feature_page("MEDIA INTELLIGENCE", "Recordings, clips, screenshots, evidence packages, and exports.", "/media"))
        self._add_page("tasks", self._feature_page("TASKS & OBJECTIVES", "Quest, hideout, and session planning will be connected in a later sprint.", None))
        self._add_page("maps", self._feature_page("MAP INTELLIGENCE", "Routes, locations, heatmaps, and encounter overlays are planned here.", None))
    ...
    def _configure_timers(self) -> None:
        ...
        self.poll_timer.setInterval(int(self.settings.desktop.poll_interval_seconds * 1000))
    ...
    def _build_tray(self) -> None:
        ...
        if not self.settings.desktop.minimize_to_tray or not QSystemTrayIcon.isSystemTrayAvailable():
            return
