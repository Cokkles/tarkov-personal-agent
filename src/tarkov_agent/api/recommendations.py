...
def attach_recommendation_routes(app: FastAPI, context: AgentContext) -> None:
    ...
    async def recommendations_index() -> FileResponse:
        return FileResponse(
            Path(__file__).parent / "static" / "recommendations.html"
        )

    @app.post(
        "/api/recommendations/generate",
        response_model=RecommendationPlan,
    )
    ...
    async def latest_recommendation() -> RecommendationPlan:
        ...
        if latest is None:
            raise HTTPException(
                status_code=404,
                detail="No recommendation plan has been generated",
            )
        return latest
    ...
    async def export_recommendation(format_name: str) -> Response:
        ...
        if format_name == "json":
            return Response(
                ...
                headers={
                    "Content-Disposition": (
                        'attachment; filename="recommendation-plan.json"'
                    )
                },
            )
        if format_name == "markdown":
            return Response(
                ...
                headers={
                    "Content-Disposition": (
                        'attachment; filename="recommendation-plan.md"'
                    )
                },
            )
        raise HTTPException(
